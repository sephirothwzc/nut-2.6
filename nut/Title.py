...
class Title:
	...
	def exportDict(self, isRegion = False, full = True):
		...
		if isRegion:
			...
			blacklist = ('isDLC', 'isUpdate', 'idExt', 'updateId', 'baseId', 'rightsId', 'key', 'isDemo', 'regions', 'nsuId', 'category', 'ratingContent', 'numberOfPlayers', 'rating', 'frontBoxArt', 'intro', 'languages', 'language', 'iconUrl', 'screenshots', 'bannerUrl')
	...
	def isActive(self, skipKeyCheck = False):
		...
		if (self.isDLC or self.isUpdate or Config.download.base) and (not self.isDLC or Config.download.DLC) and (not self.isDemo or Config.download.demo) and (not self.isUpdate or Config.download.update) and (self.key or Config.download.sansTitleKey or self.isUpdate or skipKeyCheck) and (len(Config.titleWhitelist) == 0 or self.id in Config.titleWhitelist) and self.id not in Config.titleBlacklist:
